...
import { Users, Search, MoreHorizontal, Mail, Calendar, Loader2, ChevronLeft, ChevronRight } from "lucide-react"
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
import { getUsers, User, UsersResponse, isAuthenticated, getImageUrl, sendEmail } from "@/lib/api"
...
export default function UsersPage() {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true)
      setError(null)
      console.log('Fetching users with params:', { currentPage, searchTerm, selectedStatus })
  ...
  const getInitials = (name: string | null | undefined, username?: string | null) => {
  ...
  return (
    <div className="space-y-6">
      <AnalyticsModal user={selectedUser} isOpen={analyticsModalOpen} onClose={() => setAnalyticsModalOpen(false)} />
      <div>
        <h1 className="text-3xl font-bold tracking-tight">Manage Users</h1>
        <p className="text-muted-foreground">
          View and manage all users on the platform.
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-left py-3 px-4 font-medium">Reading List</th>
                    ...
                    {users.map((user) => (
                      <tr key={user.userId} className="border-b hover:bg-gray-50">
                        <td className="py-3 px-4">
                          <div className="flex items-center gap-3">
                            <Avatar className="h-8 w-8">
                              <AvatarImage src={getImageUrl(user.pfp)} alt={user.username || user.name || 'User'} />
                              <AvatarFallback>{getInitials(user.name, user.username)}</AvatarFallback>
                            </Avatar>
                            <div>
                              <div className="font-medium">@{user.username || 'unknown'}</div>
                              <div className="text-sm text-gray-600">{user.name || 'No name'}</div>
                              <div className="text-xs text-gray-500">{user.email || 'No email'}</div>
                            </div>
                          </div>
                        </td>
                        <td className="py-3 px-4">
                          <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getStatusColor(user.status || 'offline')}`}>
                            {user.status || 'offline'}
                          </span>
                        </td>
                        <td className="py-3 px-4">
                          <div className="text-sm font-medium">{(user.follower_count || 0).toLocaleString()}</div>
                        </td>
                        <td className="py-3 px-4">
                          <div className="text-sm font-medium">{user.readingList || 0}</div>
                        </td>
                        ...
                        <DropdownMenuContent align="end" className="bg-white border border-gray-200 shadow-lg">
                          ...
                          <DropdownMenuItem onClick={() => handleViewAnalytics(user)}>
